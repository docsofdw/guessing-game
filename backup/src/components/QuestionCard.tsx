...
const QuestionCard: React.FC<QuestionCardProps> = ({ 
  ...
  const getTeamColor = (teamName?: string) => {
    ...
    return teamName && teamColors[teamName] ? teamColors[teamName] : 'bg-gray-500';
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {options && options.map((option, index) => {
            ...
            return (
              <button
                key={index}
                className={`p-4 rounded-lg text-white font-medium ${bgColor} transition-colors transform hover:scale-[1.03] active:scale-[0.98]`}
